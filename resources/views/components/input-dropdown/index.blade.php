...
<div class="relative z-10 group w-full" data-input-dropdown-component>
    ...
    <div class="absolute inset-0 top-24 border h-fit bg-white shadow-soft rounded-sm group-data-[state=open]:opacity-100 group-data-[state=open]:visible group-data-[state=open]:translate-y-0 group-data-[state=open]:scale-100  opacity-0 invisible -translate-y-4 scale-95 transition-all duration-200 p-1"
        ...
        @foreach ($items as $item)
            <div data-input-dropdown-item="{{ $item }}"
                ...
                class="flex items-center justify-between px-4 py-4 rounded-sm text-sm hover:bg-secondary/5 data-[input-dropdown-state=active]:bg-secondary/5 group animate-cta text-secondary data-[input-dropdown-state=active]:text-primary">
                <p>
                    {{ $item }}
                </p>
