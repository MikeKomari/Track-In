...
@section('content')
    ...
    <div class="flex justify-between flex-col h-full">
        ...
        <form method="POST" action="/login" class="flex flex-col px-20 justify-center max-md:px-0 max-md:w-full">
            @csrf
            <p class="text-3xl font-bold">{{ __('messages.login.header') }}</p>
            <p class="my-3 text-secondary">{{ __('messages.login.header_desc') }}</p>

            <x-input label="{{ __('messages.login.input.email') }}" placeholder="elysiabellamy@gmail.com"
                class="mt-1 w-full" name="email" :value="old('email')" :error="$errors->first('email')" />
            ...
            <div class="flex items-center gap-2 my-4">
                ...
                <label for="rememberMe">{{ __('messages.login.input.remember_me') }}</label>
            </div>

            <button type="submit"
                class="bg-accent w-full rounded-xl text-white py-4 animate-cta">{{ __('messages.login.input.submit') }}</button>
